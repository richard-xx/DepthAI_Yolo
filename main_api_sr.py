...
def main():
    ...
    with dai.Device(
        create_pipeline_color() if args.color else create_pipeline(),
        get_device_info(),
...
def create_pipeline_color():
    # Create pipeline
    pipeline = dai.Pipeline()

    # Define sources and outputs
    mono_right = pipeline.createColorCamera()
    mono_right.setBoardSocket(dai.CameraBoardSocket.CAM_C)
    mono_right.setResolution(dai.ColorCameraProperties.SensorResolution.THE_800_P)
    mono_right.setFps(60)

    if args.spatial:
        mono_left = pipeline.createColorCamera()
    ...
    # Properties
    mono_right.setPreviewSize(nn_width, nn_height)
    mono_right.setInterleaved(False)
    mono_right.setColorOrder(dai.ColorCameraProperties.ColorOrder.BGR)
    mono_right.setPreviewKeepAspectRatio(not args.fullFov)
...
def create_pipeline():
    ...
    image_manip.initialConfig.setResize(nn_width, nn_height)
    image_manip.initialConfig.setKeepAspectRatio(not args.fullFov)
